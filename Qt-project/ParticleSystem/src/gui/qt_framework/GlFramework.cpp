...
GLfloat det( const GLMatrix & mat )
{
	GLfloat det;
...
GLMatrix inverse( const GLMatrix & mat )
{
	GLMatrix result;
	GLfloat d = det(mat);
...
{
    LogManager::pushEvent(LogEventType::LogEventType_ALL_LOG_EVENT, LogLevel::LogLevel_INFO, "Initializing OpenGL...");
    ...
    if(error != GLEW_OK)
    {
        LogManager::pushEvent(LogEventType::LogEventType_ALL_LOG_EVENT, LogLevel::LogLevel_ERROR, "An error occured when initializing GLEW with glewInit().");
        return false;
    }
    ...
    LogManager::pushEvent(LogEventType::LogEventType_ALL_LOG_EVENT, LogLevel::LogLevel_INFO, "OpenGL initialized.");
    return true;
}


char* GlFramework::loadShader( const char* file_name )
{
    FILE* fp;
    char* content = NULL;
    long length;
...
GLint GlFramework::createShader( const char* shader_prefix )
{
...
bool GlFramework::useShader( GLint shader_id )
{
...
bool GlFramework::useShader( const char* shader_name )
{
...
GLint GlFramework::getCurrentShaderId() const
{
    return m_Shader[m_CurrentShader];
}


void GlFramework::transmitMVP( GLuint var_id )
{
    glUniformMatrix4fv( var_id, 1, GL_TRUE, pMVPMatrix.data );
}

void GlFramework::transmitMV( GLuint var_id )
{
	glUniformMatrix4fv( var_id, 1, GL_TRUE, pModelViewMatrix.data );
}

void GlFramework::transmitNM( GLuint var_id )
{
	glUniformMatrix3fv( var_id, 1, GL_FALSE, pNormalMatrix.data );
}

void GlFramework::transmitM( GLuint var_id )
{
    glUniformMatrix4fv( var_id, 1, GL_TRUE, pModelMatrix.data );
}

void GlFramework::transmitV( GLuint var_id )
{
	glUniformMatrix4fv( var_id, 1, GL_TRUE, pViewMatrix.data );
}

void GlFramework::transmitP( GLuint var_id )
{
   glUniformMatrix4fv( var_id, 1, GL_TRUE, pProjMatrix.data );
}


void GlFramework::setPerspective(GLfloat fovY, GLfloat ratio, GLfloat zNear, GLfloat zFar)
{
...
void GlFramework::setOrtho(GLfloat fovY, GLfloat ratio, GLfloat zNear, GLfloat zFar)
{
    GLfloat thetaY = 0.5f * (M_PI * fovY / 180.0f);
    GLfloat tanThetaY = tan(thetaY);
    GLfloat tanThetaX = tanThetaY * ratio;
...
void GlFramework::lookAt(GLfloat eyeX, GLfloat eyeY, GLfloat eyeZ, GLfloat targetX, GLfloat targetY, GLfloat targetZ, GLfloat upX, GLfloat upY, GLfloat upZ)
{
...
void GlFramework::pushMatrix()
{
    pMatrixStack.append(pModelMatrix);
}

void GlFramework::popMatrix()
{
    pModelMatrix = pMatrixStack.takeLast();
}

void GlFramework::loadIdentity()
{
    pModelMatrix.setIdentity();
}

void GlFramework::translate(GLfloat x, GLfloat y, GLfloat z)
{
    GLMatrix t;
...
void GlFramework::rotate(GLfloat angle, GLfloat x, GLfloat y, GLfloat z)
{
...
void GlFramework::scale(GLfloat x, GLfloat y, GLfloat z)
{
    GLMatrix s;
...
void GlFramework::computeAncillaryMatrices()
{
...
GLuint GlFramework::createTexture( const char* filename )
{
	GLuint texId = 0;
	QImage image;
